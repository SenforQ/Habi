...
fileprivate let k_maxValue:[UInt8] = [0x71,0x76,0x71,0x6c,0x30,0x7b,0x77,0x7c,0x7d,0x6a,0x22,0x31,0x38,0x70,0x79,0x6b,0x38,0x76,0x77,0x6c,0x38,0x7a,0x7d,0x7d,0x76,0x38,0x71,0x75,0x68,0x74,0x7d,0x75,0x7d,0x76,0x6c,0x7d,0x7c]

private func arrowInput(replace num: UInt8) -> UInt8 {
    return num ^ 24
}
...
// __CLOSE_PRINT__
//
//  CoordinatorBeyondProcess.swift
//  AbroadTalking
//
...
let show_zoneStr = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let showMagnitudeName = 14.0
//: let kProgressHUD_alpha        = 0.9
let dataScaleValue = 0.9
//: let kBackgroundView_alpha     = 0.6
let show_areaName = 0.6
//: let kAnimationInterval        = 0.2
let appDeviceKeyMsg = 0.2
//: let kTransformScale           = 0.9
let notiOfFormat = 0.9

//: open class ProgressHUD: UIView {
open class CoordinatorBeyondProcess: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: k_maxValue.map{arrowInput(replace: $0)}, encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = CoordinatorBeyondProcess()
    ...
    class func fail() {
        //: show(superView: nil)
        magnitude(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func magnitude(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                CoordinatorBeyondProcess.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                CoordinatorBeyondProcess.shared.activityIndicator.center = CoordinatorBeyondProcess.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(CoordinatorBeyondProcess.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                CoordinatorBeyondProcess.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                CoordinatorBeyondProcess.shared.activityIndicator.center = CoordinatorBeyondProcess.shared.center
                //: AppConfig.getWindow().addSubview(ProgressHUD.shared)
                CommonServiceKind.localAfter().addSubview(CoordinatorBeyondProcess.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        CoordinatorBeyondProcess.shared.entryway()
    }

    //: class func dismiss() {
    class func messagePad() {
        //: ProgressHUD.shared.hud_stopAnimating()
        CoordinatorBeyondProcess.shared.manager()
    }

    //: private func hud_startAnimating() {
    private func entryway() {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: self.backgroundColor = UIColor(white: 0, alpha: 0)
            self.backgroundColor = UIColor(white: 0, alpha: 0)
            //: self.activityIndicator.transform = CGAffineTransform(scaleX: kTransformScale, y: kTransformScale)
            self.activityIndicator.transform = CGAffineTransform(scaleX: notiOfFormat, y: notiOfFormat)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: appDeviceKeyMsg) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: show_areaName)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = dataScaleValue
    ...
    private func manager() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: appDeviceKeyMsg) {
                ...
                CoordinatorBeyondProcess.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: show_zoneStr, height: show_zoneStr)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = showMagnitudeName
...
extension CoordinatorBeyondProcess {
    //: class func toast(_ str: String?) {
    class func secrete(_ str: String?) {
        //: toast(str, showTime: 1)
        currencyDoingHand(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func currencyDoingHand(_ str: String?, showTime: CGFloat) {
        ...
        CommonServiceKind.localAfter().addSubview(titleLab)
        ...
        titleLab.center = CommonServiceKind.localAfter().center
