...
/*: "USD" :*/
fileprivate let dataPanelUrl:String = "receiveD"
...
class MemoryDataManager: NSObject {
    //: static let shared = AppAdjustManager()
    static let shared = MemoryDataManager()
    ...
    func remove() {
        ...
        let adjustConfig = ADJConfig(appToken: show_zoneMessage, environment: environment)
        ...
        MemoryDataManager.clear(token: user_scaleMagnitudeValue)
    }
}
...
extension MemoryDataManager: AdjustDelegate {
    /// 获取设备id
    //: class func getAdjustAdid() -> String {
    class func belowSource() -> String {
    ...
    class func commonPolicy(token: String, count: Double) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: event?.setRevenue(count, currency: "USD")
        event?.setRevenue(count, currency: (dataPanelUrl.replacingOccurrences(of: "receive", with: "US")))
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
    }
    ...
    class func overDeadline(token: String) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
